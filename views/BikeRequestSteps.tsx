...
> {
  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      if (permissionStatus === true) {
        Geolocation.getCurrentPosition(
          position => {
            ...
            console.log('LATIUTISE :: ', latitude, longitude);
      ...
      if (currentActivity != null) {
        ...
        console.log('FETCH STATUS ::: ', vehicleFetchStatus);
        this.setState({
  ...
  handlePaymentProcess = async () => {
    ...
    //   });
    this.props.navigation.push('DashBoardView');
  };
  ...
  fetchBikeBrands = async () => {
    try {
      this.setState({loading: true});
      const response = await getBikeBrands();
      console.log('BIKESSSS :: ', response.data.data);
      this.setState({bikeBrands: response.data.data, loading: false});
    } catch (error) {
  ...
  getVariousBikeDetails = async (bike: any) => {
    this.setState({loading: true});
    getBikeDetailsList(bike._id || '')
      .then((response: any) => {
        console.log(
          'respon  of brand api',
          JSON.stringify(response.data.data.models),
        );
        this.setState({
          selectedBikeBrand: bike.brand,
          bikeList: response.data.data.models,
  ...
  handleConfirmation = async () => {
    ...
    getBikeProblems()
      .then((response: any) => {
        ...
        const bikeProblemsLabel = response.data.data!.map((item: any) => {
          console.log('item in data ::::: ', item);
          return {value: item.problemname, label: item.problemname};
        });
        console.log('Bike problem label :::::: ', bikeProblemsLabel);
        this.setState({
      ...
      .catch(error => errorMessage('Something went wrong', error));
    this.setState({loading: false});
  };
      ...
      return (
        <View style={styles.container}>
          <ScrollView contentContainerStyle={styles.scrollContainer}>
            ...
            <ScrollView contentContainerStyle={styles.listContainer}>
              {this.state.bikeBrands.map((bike: any, index: number) => (
                <View style={styles.listItem} key={index}>
                  <View style={styles.itemIcon} />
                  <TouchableOpacity
                    onPress={() => {
